...
public class RevalidationMiddleware
{
        ...
        ILogger<RevalidationMiddleware> logger)
    {
        ...
        _logger = logger;
        // Trim any trailing slash so the query separator yields ".../api/revalidate?secret="
        // (a trailing "/" would make Next 308-redirect and drop the POST).
        _url = (config["Revalidate:Url"] ?? Environment.GetEnvironmentVariable("SITE_REVALIDATE_URL"))?.TrimEnd('/');
