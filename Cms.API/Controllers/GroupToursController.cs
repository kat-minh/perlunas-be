...
public class GroupToursController : ControllerBase
{
    private readonly TourCardsService.IService _service;
    public GroupToursController(TourCardsService.IService service) => _service = service;

    [HttpGet]
    [AllowAnonymous]
    public async Task<IActionResult> GetAll()
    {
        var result = await _service.GetAllAsync(TourCardType.Group);
    ...
    {
        var result = await _service.GetByIdAsync(id, TourCardType.Group);
        return Ok(ApiResponseFactory.Base(result, true, "", HttpContext.TraceIdentifier));
    }
    ...
    {
        request.Type = TourCardType.Group;
    ...
    {
        var result = await _service.UpdateAsync(id, request, TourCardType.Group);
        return Ok(ApiResponseFactory.Base(result, true, "", HttpContext.TraceIdentifier));
    }
    ...
    {
        await _service.DeleteAsync(id, TourCardType.Group);
